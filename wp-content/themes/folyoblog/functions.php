...
set_post_thumbnail_size( 120, 120, true ); // default Post Thumbnail dimensions (cropped)
add_image_size( 'post-thumb', 630, 150 ); //300 pixels wide (and unlimited height)
...
function sg_post_thumbnail($id, $link=""){
	if ( has_post_thumbnail($id) ) { // check if the post has a Post Thumbnail assigned to it.
		?>
		<div class="entry-image">
			<a href="<?php if($link){echo $link;}else{echo get_permalink($id);} ?>">
				<span class="arrow"></span>
				<!-- <span class="entry-date">
									<span class="month"><?php echo get_the_time("M", $id); ?></span>
									<span class="day"><?php echo get_the_time("n", $id); ?></span>
								</span> -->
...
function select_menu(){
	...
	if ( ( $locations = get_nav_menu_locations() ) && isset( $locations[ $menu_name ] ) ) {
		...
		$menu_list = '<select id="selectmenu-' . $menu_name . '">';
		foreach ( (array) $menu_items as $key => $menu_item ) {
			...
			$menu_list .= '<option '.$selected.' value="' . $url . '">' . $title . '</option>';
		}
		$menu_list .= '</select>';
		echo $menu_list;
	}
